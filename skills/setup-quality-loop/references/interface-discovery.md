...
1. **MCP tier:** if a Greptile MCP server is registered in this harness, list its tools and read their descriptions — that listing, not memory, defines what review-lifecycle data, comment metadata, and guidance-management operations exist right now.
2. **CLI tier:** check for a Greptile CLI on PATH; if present, its help output defines its current verbs.
3. **Host-API tier (always present):** Greptile ultimately acts as a reviewer identity on the repository host. Everything essential — reading review comments, replying in threads, adding reactions, mention-triggering a review — works through the host's ordinary PR interfaces even with no Greptile-specific channel at all.
4. **Reviewer identity:** identify the review bot by pattern-matching comment authors on a recent reviewed PR (a bot-suffixed account whose name contains the vendor's), never by a hardcoded login — app slugs differ between installations and over time. On a fresh setup with no reviewed PR yet, defer identification to the first triage.
5. **Absence semantics:** a missing tier is degraded fidelity, not an error. Note what the session lost (e.g., without MCP there is no programmatic review-lifecycle visibility — completion and skips must be inferred from host-side signals) and proceed on the tiers that exist.
